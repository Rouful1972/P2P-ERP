...
const BOMManagement = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    bpr_eff_dt: '',
    bcr_version_no: '',
    bcr_eff_dt: new Date().toISOString().split('T')[0], // Default to today's date
    opl_p_code: '',
    batch_per_large_unit: 1.0,
    batch_per_large_qnty: 1.0,
    note: '',
    initiator: '',
    is_active: true,
    details: []
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const sanitizedFormData = {
        ...formData,
        bom_ratio: parseFloat(formData.bom_ratio) || 1.0,
        annex_ratio: parseFloat(formData.annex_ratio) || 1.0,
        batch_qnty: parseFloat(formData.batch_qnty) || 1.0,
        per_unit_wt: parseFloat(formData.per_unit_wt) || 1.0,
        std_avg_wt: parseFloat(formData.std_avg_wt) || 1.0,
        label_ratio: parseFloat(formData.label_ratio) || 1.0,
        batch_per_large_unit: parseFloat(formData.batch_per_large_unit) || 1.0,
        batch_per_large_qnty: parseFloat(formData.batch_per_large_qnty) || 1.0,
        bcr_eff_dt: formData.bcr_eff_dt || new Date().toISOString().split('T')[0],
        details: formData.details.map(detail => ({
          ...detail,
          pcat_code: detail.pcat_code || formData.pcat_code, // Ensure pcat_code is set
          p_code: detail.p_code || formData.product_code, // Ensure p_code is set
          declared_qnty: parseFloat(detail.declared_qnty) || 1.0,
          qty_per_batch: parseFloat(detail.qty_per_batch) || 1.0,
          overage: parseFloat(detail.overage) || 0.0,
          seq: parseInt(detail.seq) || 1
        }))
      };
      
      // Validate that all details have required fields
      const invalidDetails = sanitizedFormData.details.filter(detail => 
        !detail.pcat_code || !detail.p_code || !detail.raw_code
      );
      
      if (invalidDetails.length > 0) {
        console.error('Invalid details found:', invalidDetails);
        throw new Error('Some BOM details are missing required fields (pcat_code, p_code, or raw_code)');
      }
      
      // Debug logging for payload verification
      console.log('Final sanitized payload:', sanitizedFormData);
      console.log('Details with pcat_code and p_code:', sanitizedFormData.details.map(d => ({
        raw_code: d.raw_code,
        pcat_code: d.pcat_code,
        p_code: d.p_code
      })));
  ...
  const addBomDetail = () => {
    // First validate that parent form has required data
    if (!formData.pcat_code || !formData.product_code) {
      toast({
        title: 'Missing Parent Information',
        description: 'Please select Product Category and Product Code in the Basic Information tab first',
        status: 'error',
        duration: 5000,
        isClosable: true,
      });
    ...
    if (!bomDetail.raw_code || !bomDetail.qty_per_batch) {
      toast({
        title: 'Missing Information',
        description: 'Please select raw material and enter quantity per batch',
        status: 'warning',
        duration: 3000,
        isClosable: true,
      });
    ...
    // Debug logging
    console.log('formData.pcat_code:', formData.pcat_code);
    console.log('formData.product_code:', formData.product_code);
    console.log('bomDetail before update:', bomDetail);

    const detailWithDefaults = {
      ...bomDetail,
      pcat_code: formData.pcat_code, // Inherit from main form
      p_code: formData.product_code, // Inherit from main form
      declared_qnty: parseFloat(bomDetail.declared_qnty) || 1.0,
      qty_per_batch: parseFloat(bomDetail.qty_per_batch) || 1.0,
      overage: parseFloat(bomDetail.overage) || 0.0,
      seq: parseInt(bomDetail.seq) || (formData.details.length + 1)
    };

    console.log('detailWithDefaults:', detailWithDefaults);

    // Validate that the inherited values are not empty
    if (!detailWithDefaults.pcat_code || !detailWithDefaults.p_code) {
      toast({
        title: 'Data Inheritance Error',
        description: 'Failed to inherit product category and product code. Please refresh and try again.',
        status: 'error',
        duration: 5000,
        isClosable: true,
      });
    ...
    setFormData({
      ...formData,
      details: [...formData.details, detailWithDefaults]
    });
    setBomDetail({
  ...
  const handleAdd = () => {
    ...
    setFormData({
      ...
      bpr_eff_dt: '',
      bcr_version_no: '',
      bcr_eff_dt: new Date().toISOString().split('T')[0],
      opl_p_code: '',
      batch_per_large_unit: 1.0,
      batch_per_large_qnty: 1.0,
      note: '',
                ...
                <TabPanels>
                  ...
                  <TabPanel>
                    <Grid templateColumns="repeat(3, 1fr)" gap={4}>
                      ...
                      <GridItem>
                        <FormControl>
                          <FormLabel>BOM Ratio</FormLabel>
                          <NumberInput step={0.01} precision={2}>
                            <NumberInputField
                              value={formData.bom_ratio}
                              onChange={(e) => setFormData({...formData, bom_ratio: parseFloat(e.target.value) || 1.0})}
                            />
                          </NumberInput>
                        </FormControl>
                      </GridItem>
                      ...
                      <GridItem>
                        <FormControl>
                          <FormLabel>Annex Ratio</FormLabel>
                          <NumberInput step={0.01} precision={2}>
                            <NumberInputField
                              value={formData.annex_ratio}
                              onChange={(e) => setFormData({...formData, annex_ratio: parseFloat(e.target.value) || 1.0})}
                            />
                          </NumberInput>
                        </FormControl>
                      </GridItem>
                      ...
                      <GridItem>
                        <FormControl>
                          ...
                          <NumberInput step={0.01} precision={2}>
                            <NumberInputField
                              value={formData.batch_qnty}
                              onChange={(e) => setFormData({...formData, batch_qnty: parseFloat(e.target.value) || 1.0})}
                            />
                          </NumberInput>
                        </FormControl>
                      </GridItem>
                      ...
                      <GridItem>
                        <FormControl>
                          <FormLabel>Per Unit Weight</FormLabel>
                          <NumberInput step={0.01} precision={2}>
                            <NumberInputField
                              value={formData.per_unit_wt}
                              onChange={(e) => setFormData({...formData, per_unit_wt: parseFloat(e.target.value) || 1.0})}
                            />
                          </NumberInput>
                        </FormControl>
                      </GridItem>
                      ...
                      <GridItem>
                        <FormControl>
                          <FormLabel>Standard Average Weight</FormLabel>
                          <NumberInput step={0.01} precision={2}>
                            <NumberInputField
                              value={formData.std_avg_wt}
                              onChange={(e) => setFormData({...formData, std_avg_wt: parseFloat(e.target.value) || 1.0})}
                            />
                          </NumberInput>
                        </FormControl>
                      </GridItem>
                      ...
                      <GridItem>
                        <FormControl>
                          <FormLabel>Label Ratio</FormLabel>
                          <NumberInput step={0.01} precision={2}>
                            <NumberInputField
                              value={formData.label_ratio}
                              onChange={(e) => setFormData({...formData, label_ratio: parseFloat(e.target.value) || 1.0})}
                            />
                          </NumberInput>
                        </FormControl>
                      </GridItem>
                      ...
                      <GridItem>
                        <FormControl>
                          <FormLabel>Batch per Large Unit</FormLabel>
                          <NumberInput step={0.01} precision={2}>
                            <NumberInputField
                              value={formData.batch_per_large_unit}
                              onChange={(e) => setFormData({...formData, batch_per_large_unit: parseFloat(e.target.value) || 1.0})}
                            />
                          </NumberInput>
                        </FormControl>
                      </GridItem>
                      ...
                      <GridItem>
                        <FormControl>
                          <FormLabel>Batch per Large Quantity</FormLabel>
                          <NumberInput step={0.01} precision={2}>
                            <NumberInputField
                              value={formData.batch_per_large_qnty}
                              onChange={(e) => setFormData({...formData, batch_per_large_qnty: parseFloat(e.target.value) || 1.0})}
                            />
                          </NumberInput>
                        </FormControl>
                      </GridItem>
                  ...
                  <TabPanel>
                    <Grid templateColumns="repeat(3, 1fr)" gap={4}>
                      ...
                      <GridItem>
                        <FormControl>
                          <FormLabel>BCR Effective Date</FormLabel>
                          <Input
                            type="date"
                            value={formData.bcr_eff_dt}
                            onChange={(e) => setFormData({...formData, bcr_eff_dt: e.target.value})}
                          />
                        </FormControl>
                      </GridItem>
        ...
        <ModalContent>
          ...
          <ModalBody pb={6}>
            <VStack spacing={4}>
              ...
              <FormControl isRequired>
                <FormLabel>Declared Quantity</FormLabel>
                <NumberInput step={0.01} precision={2}>
                  <NumberInputField
                    value={bomDetail.declared_qnty}
                    onChange={(e) => setBomDetail({...bomDetail, declared_qnty: parseFloat(e.target.value) || 1.0})}
                  />
                </NumberInput>
              </FormControl>
